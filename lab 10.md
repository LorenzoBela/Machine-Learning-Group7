...
- Set up a Raspberry Pi with the operating system, Python environment, and libraries required to run TensorFlow Lite inference on the edge (tflite_runtime, camera driver, GPIO library).
...
- Set input tensor: copy the preprocessed image into the Interpreter's input buffer (set_tensor).
- Invoke: run inference (invoke()). This is the step whose latency is measured for benchmarking.
- Read and decode output: read the output tensor(s) (get_tensor) and apply the task-appropriate decoding - argmax for classification, non-maximum suppression for detection, or pixel-class counting for segmentation.
...
- tflite_runtime - the lightweight TFLite inference library.
...
- Install tflite_runtime using pip. Make sure the wheel matches the system Python version and the aarch64 architecture.
...
- Verify the install by importing tflite_runtime.Interpreter and gpiozero in a Python shell.
...
- Copy the deployment_package/ directory from the development machine to /home/&lt;user&gt;/lab10_deployment/ on the Raspberry Pi. Use scp, rsync, or a USB drive.
- Confirm that all five files are present: model.tflite, labels.txt, preprocessing.txt, model_card.md, sample_input.jpg.
- Run a sanity check: load model.tflite with the Interpreter, run one inference on sample_input.jpg (preprocessed exactly per preprocessing.txt), and compare the predicted class to the value documented in model_card.md. The prediction must match before proceeding.
...
- Why is tflite_runtime preferred over the full tensorflow package on a Raspberry Pi? Address binary size, dependency footprint, and runtime performance in your answer.
- Describe how you wired one LED and the push-button. Justify the 330 Ω current-limiting resistor for the LED and explain why no external resistor is required for the button.
- Explain how the LED-output mapping differs across classification, object detection, and segmentation. Refer specifically to your group's task type and the rule you implemented in Part F.
- The first inference on the Pi is typically slower than subsequent ones. Explain why, and describe the technique you used in Part E to keep the user from experiencing this cold-start latency.
- Compare the on-Pi inference latency to the dev-machine latency you measured in Lab 9. What specific factors account for the difference, and what design choice in Labs 6-8 had the biggest influence on the latency observed here?
